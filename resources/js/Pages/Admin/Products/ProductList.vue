...
let isAddProduct = ref(false);
const editMode= ref(false);

let productImages = ref([]);
let dialogVisible= ref(false);
let dialogImageUrl = ref('');
let dialogImgVisible= ref('')
...
const AddProduct= async()=>{
    ...
    product_images: productImages.value.map(image => image.raw)});
...
const updateProduct= async ()=>{
    ...
    product_images: productImages.value.map(image => image.raw)});
    try{
        await router.post('products/update/'+id.value,form,{
            onSuccess: page=>{
                dialogVisible.value = false;
                resetForm();
                Swal.fire({
...
<template>
  ...
  <div class="relative z-0 w-full mb-5 group">
      <input type="text" v-model="title" name="floating_title" id="floating_title" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
  ...
  <div class="relative z-0 w-full mb-5 group">
      <input type="text" v-model="price" name="floating_price" id="floating_price" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
  ...
  <div class="relative z-0 w-full mb-5 group">
      <input v-model="quantity" type="number" name="quantity" id="floating_quantity" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
...
<!-- end -->
  ...
  <div class="mt-2">
    ...
    <select v-model="brand_id" id="categories" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
...
<!-- preview product on edit -->
<div class="flex">

    <div class="relative my-3" v-for="(pimage,index) in product_images" :key="pimage.id">
        <img class="w-32 h-32 rounded mx-2" :src="`/${pimage.image}`" alt="">
        <span 
        @click="deleteImage(pimage,index)"
        class="absolute cursor-pointer top-0 right-0 transform -translate-y-1/2 w-3.5 h-3.5 bg-red-400 border-2 border-white dark:border-gray-800 rounded-full">
            <span class="text-white text-xs font-bold absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">x</span>
        </span>
    </div>
</div>
...
<div class="flex justify-end">
    <button type="submit" class="focus:outline-none mt-4  text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Add Product</button>
            ...
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <button @click="openAddModal"
                        ...
                        <svg class="h-5 w-5 mr-2 font-bold" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" >
                    ...
                    <tbody>
                        ...
                        <tr v-for="product in products" :key="product.id" class="border-b dark:border-gray-700">
                            ...
                            <span v-if="product.inStock" class=" w-18 bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-2 rounded dark:bg-green-900 dark:text-green-300">  
                                        InStock</span>
                                <span v-else class=" w-18 bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-2
                                rounded dark:bg-red-900 dark:text-red-300">Out of Stock</span>
                            </td>

                            <td class="px-4 py-3">
                                <button v-if="product.published" class=" w-28 bg-primary-100 hover:bg-white text-primary-600  font-semibold py-2 px-4 border
                             border-primary-600 rounded shadow">
                                 Published</button>
                                <button v-else class= "w-28 bg-red-100 hover:bg-white  text-red-400 font-semibold py-2 px-4 border
                             border-red-400 rounded shadow">
                                 unPublished</button>
                            </td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <div :id="`${product.id}`" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" :aria-labelledby="`${product.id}-button`">
                                        ...
                                        <li>
                                            <button @click="openEditModal(product)" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full text-left">Edit</button>
                                        </li>
                                    </ul>
                                    <div class="py-1">
